...
--}}
NOSOTROS <strong>{{ $config->patrono_nombre ?: '______________________' }}</strong>, de {{ $config->patrono_edad ?? '____' }} años de edad,
...
y <strong>{{ $empleado->name }}</strong>, de {{ $perfil->edad ?? '____' }} años de edad, {{ $perfil->profesion_oficio ?: 'empleado' }}, del domicilio del distrito de
...
<strong>A) CLASE DE TRABAJO O SERVICIO</strong>: El trabajador se obliga a prestar sus servicios al patrono como: {{ $perfil->cargo ?: '____________' }}
Desempeñando las funciones de: {{ $perfil->cargo ?: '____________' }}
<strong>B) DURACIÓN DEL CONTRATO Y TIEMPO DE SERVICIO</strong>: El presente Contrato se celebrará por:
...
expresión de causa.
<strong>C) LUGAR DE PRESTACIÓN DE SERVICIOS Y DE ALOJAMIENTO</strong>:
El lugar de prestación de los servicios será: {{ $lugar ?: '____________' }}
<strong>D) HORARIO DE TRABAJO: </strong>@if($perfil->horario_laboral){{ $perfil->horario_laboral }}@else Del día ________________ al día _________________, de _____________, a _______________
...
<strong>H) OTRAS ESTIPULACIONES</strong>: {{ rtrim($perfil->otras_estipulaciones ?: '____________', '.') }}.
<strong>I)</strong> En el presente Contrato Individual de Trabajo se entenderán incluidos, según el caso, los derechos y deberes laborales
...
<strong>J)</strong> Este contrato sustituye cualquier otro Convenio Individual de Trabajo anterior, ya sea escrito o verbal, que haya estado vigente
