...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { text-align: center; margin-bottom: 20px; border-bottom: 3px solid #f97316; padding-bottom: 10px; }
        ...
        .info-item { background: #f9fafb; padding: 8px 10px; border-left: 3px solid #f97316; border-radius: 2px; }
        ...
        .vendedor-header { background: #374151; color: white; padding: 8px; margin: -10px -10px 10px -10px; border-radius: 2px 2px 0 0; }
        ...
        .summary-box { background: #fef3c7; border: 1px solid #fcd34d; padding: 8px; margin-top: 10px; border-radius: 2px; }
        ...
        .status-badge { display: inline-block; padding: 2px 6px; border-radius: 2px; font-size: 8px; font-weight: bold; }
        .status-activa { background: #dbeafe; color: #0c4a6e; }
        .status-liquidada { background: #dcfce7; color: #166534; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>REPORTE DE ASIGNACIÓN — VENDEDOR</h1>
            <p>Fecha: {{ $fecha->format('d/m/Y') }}</p>
        </div>

        @php
            $vendedor = $asignaciones->first()?->vendedor;
            $totalVendido = $asignaciones->sum('total_vendido');
            $totalDevuelto = $asignaciones->sum('total_devuelto_valor');
        @endphp

        <div class="info-section">
            <div class="info-item">
                <div class="info-label">Vendedor</div>
                <div class="info-value">{{ $vendedor?->nombre_completo ?? '—' }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Asignaciones del día</div>
                <div class="info-value">{{ $asignaciones->count() }}</div>
            </div>
            <div class="info-item">
                <div class="info-label">Estado</div>
                <div class="info-value">{{ $asignaciones->where('estado', 'liquidada')->count() }} liquidada(s) / {{ $asignaciones->where('estado', 'activa')->count() }} activa(s)</div>
            </div>
        </div>

        @if($asignaciones->isEmpty())
            <div class="no-data">Este vendedor no tiene asignaciones para el día de hoy.</div>
        @else
            <div class="vendedor-section">
                <div class="vendedor-header">
                    <h3>{{ $vendedor?->nombre_completo }}</h3>
                    <div class="vendedor-info">
                        {{ $vendedor?->codigo ?? 'S/C' }} — Sucursal: {{ $asignaciones->first()->sucursal->nombre }}
                    </div>
                </div>
